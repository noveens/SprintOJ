        ...
        this.uploadFileToUrl = function(file, uploadUrl){
           ...
           .success(function(data){
            ...
            if(data == "No files were uploaded."){
              localStorage.setItem("messageSome",'Please upload a file to continue');

            }
            else{
              localStorage.setItem("messageSome",'');
              console.log(data)
...
///////////////////// cheking score to send to api addScore to update score/////////////
              ...
              if(Number((s.match(/1/g) || []).length) > Number((o.match(/1/g) || []).length)){
                var link = '/addScore?name='+localStorage.getItem('storageName')+'&ques=some&str='+s;
                $http.get(link)
                .success(function(response){
      ...
      $scope.language="";
        $scope.uploadFile = function(){
           ...
           var uploadUrl = "/upload?lang="+$scope.language;
            if(file!=undefined){
              ...
              if(ext == "c" || ext == "cpp"){
                fileUpload.uploadFileToUrl(file, uploadUrl);
                $scope.messageSome='';
              }else{$scope.messageSome='You can only submit .c or .cpp files.'}
            }
            else{
              $scope.messageSome='Please upload a file to continue'; 
            }
        };
         ...
         $scope.getStatus = function(){
          ...
          $http.get('/getScore?name='+localStorage.getItem("storageName")+'&ques=some')
          .success(function(response){
            console.log(response);
            console.log(localStorage.getItem("storageName"))
            oldScores = response;
            if(response[1]=='-'){document.getElementById('some').src="na.png";$scope.score=0.0;}
            else if(response[1]=='0' && response[2]=='0' & response[3]=='0'){document.getElementById('some').src="redCross.png";$scope.score=0.0;}
            else if(response[1]=='1' && response[2]=='1' & response[3]=='1'){document.getElementById('some').src="greenTick.jpg";$scope.score=100;}
            else{document.getElementById('some').src="alert.png";if(Number(response[1])+Number(response[2])+Number(response[3])==1){$scope.score=33.3}else{$scope.score=66.6}}
          })
          ;
        };
        $scope.getStatus();
