...
///////////////////// cheking score to send to api addScore to update score/////////////
              ...
              if(Number((s.match(/1/g) || []).length) > Number((o.match(/1/g) || []).length)){
                var link = '/addScore?name='+localStorage.getItem('storageName')+'&ques=chess&str='+s;
                $http.get(link)
                .success(function(response){
     ...
     myApp.controller('ProjectController', ['$scope', '$http', 'fileUpload', function($scope, $http,fileUpload){
      $scope.language="";
              ...
              var ext = fileName.split('.').pop();
               if(ext == "c" && $scope.language == "C" ){
                ...
                else if($scope.language == "")
                {
                        ...
                        $scope.messageAston = 'Please select a language';
                }
                else{$scope.messageAston='You have submitted a wrong file. Please Check again!'}
            }
            else{
        ...
        $scope.getTop = function(){
          $http.get('/getScoreQues?qname=chess')
          .success(function(response){
