        ...
        this.uploadFileToUrl = function(file, uploadUrl){
           ...
           .success(function(data){
            ...
            if(data == "No files were uploaded."){
              localStorage.setItem("messageUndefined",'Please upload a file to continue');

            }
            else{
              localStorage.setItem("messageUndefined",'');
              console.log(data)
...
///////////////////// cheking score to send to api addScore to update score/////////////
              ...
              if(Number((s.match(/1/g) || []).length) > Number((o.match(/1/g) || []).length)){
                var link = '/addScore?name='+localStorage.getItem('storageName')+'&ques=undefined&str='+s;
                $http.get(link)
                .success(function(response){
      ...
      $scope.language="";
        $scope.uploadFile = function(){
           ...
           var uploadUrl = "/upload?lang="+$scope.language;
            if(file!=undefined){
              ...
              if(ext == "c" && $scope.language == "C" ){
                ...
                {
                        $scope.messageUndefined = 'Please select a language'
                }
                else{$scope.messageUndefined='You have submitted a wrong file. Please Check again!'}
            }
            else{
              $scope.messageUndefined='Please upload a file to continue'; 
            }
        };
         ...
         $scope.getStatus = function(){
          ...
          $http.get('/getScore?name='+localStorage.getItem("storageName")+'&ques=undefined')
          .success(function(response){
            console.log(response);
            console.log(localStorage.getItem("storageName"))
            oldScores = response;
            if(response[1]=='-'){document.getElementById('undefined').src="na.png";$scope.score=0.0;}
            else if(response[1]=='0' && response[2]=='0' & response[3]=='0'){document.getElementById('undefined').src="redCross.png";$scope.score=0.0;}
            else if(response[1]=='1' && response[2]=='1' & response[3]=='1'){document.getElementById('undefined').src="greenTick.jpg";$scope.score=100;}
            else{document.getElementById('undefined').src="alert.png";if(Number(response[1])+Number(response[2])+Number(response[3])==1){$scope.score=33.3}else{$scope.score=66.6}}
          })
          ;
        };
        $scope.getStatus();
        ...
        $scope.getTop = function(){
          $http.get('/getScoreQues?qname=undefined')
          .success(function(response){
            console.log(response);
