...
///////////////////// cheking score to send to api addScore to update score/////////////
              ...
              if(Number((s.match(/1/g) || []).length) > Number((o.match(/1/g) || []).length)){
                var link = '/addScore?name='+localStorage.getItem('storageName')+'&ques=grasshopper&str='+s;
                $http.get(link)
                .success(function(response){
            ...
            if(file!=undefined){
              ...
              var ext = fileName.split('.').pop();
               if(ext == "c" && $scope.language == "C" ){
                ...
                {
                        $scope.messageOstap = 'Please select a language'  
                }
                else{$scope.messageOstap='You have submitted a wrong file. Please Check again!'}
            }
            else{
